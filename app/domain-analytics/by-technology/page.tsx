...
import { Code, Download, Loader2, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function ByTechnologyPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/domain-analytics/by-technology', {
        method: 'POST',
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `domains-by-technology-${technology.trim()}.csv`;
    a.click();
      ...
      <div className="ml-64 min-h-screen bg-[#0a0a0a]">
        <div className="p-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            <Code className="h-6 w-6 text-[#EF5744]" />
            <h1 className="text-2xl font-bold text-white">Domains by Technology</h1>
          </div>
          <p className="text-[#a1a1aa] mb-8">
            Find domains that use a specific technology (e.g. Shopify, WordPress, React).
          </p>
          ...
          <form onSubmit={handleSearch} className="bg-[#141414] border border-[#2a2a2a] rounded p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm text-[#a1a1aa] mb-1.5">Country (ISO code, optional)</label>
                <input
                  type="text"
                  value={country}
          ...
          {/* Empty state */}
          {!loading && !error && results.length === 0 && technology && (
            <div className="text-center py-12 text-[#8b8b93] text-sm">
              No results. Try a different technology name.
            </div>
          )}
        </div>
      </div>
