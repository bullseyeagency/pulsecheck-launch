...
export default function SearchVolumePage() {
  ...
  const [results, setResults] = useState<SearchVolumeResult[]>([]);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/keywords-data/search-volume', {
        method: 'POST',
  ...
  const exportCSV = () => {
    ...
    const headers = ['Keyword', 'Search Volume', 'Competition', 'Competition Index', 'CPC', 'Monthly Searches'];
    const rows = results.map((r) => [
      ...
      r.competition_index ?? '',
      r.cpc ?? '',
      (r.monthly_searches || []).map((m) => `${m.year}-${String(m.month).padStart(2, '0')}:${m.search_volume}`).join('; '),
    ]);
    ...
    a.href = url;
    a.download = 'search-volume.csv';
    a.click();
    URL.revokeObjectURL(url);
  };

  const renderTrend = (monthly: SearchVolumeResult['monthly_searches']) => {
    if (!monthly?.length) return '-';
    const max = Math.max(...monthly.map((m) => m.search_volume));
    if (max === 0) return '-';
    return (
      <div className="flex items-end gap-px h-6">
        {monthly.slice(-12).map((m, i) => (
          <div
            key={i}
            className="w-1.5 bg-[#EF5744] rounded-t opacity-80"
            style={{ height: `${Math.max(2, (m.search_volume / max) * 24)}px` }}
            title={`${m.year}-${String(m.month).padStart(2, '0')}: ${m.search_volume.toLocaleString()}`}
          />
        ))}
      </div>
    );
      ...
      <div className="ml-64 min-h-screen bg-[#0a0a0a]">
        <div className="p-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-2xl font-bold text-white">Search Volume</h1>
          </div>
          <p className="text-[#a1a1aa] mb-8">
            Get monthly search volume, competition, and CPC data for up to 50 keywords.
          </p>
          ...
          <form onSubmit={handleSearch} className="bg-[#141414] border border-[#2a2a2a] rounded p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                <label className="block text-sm text-[#a1a1aa] mb-1.5">Keywords (one per line, max 50)</label>
                <textarea
                  value={keywordsInput}
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              >
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Get Search Volume'}
              </button>
              {results.length > 0 && (
                <button
                  type="button"
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-[#1a1a1a] text-left">
                      ...
                      <th className="px-4 py-3 text-[#8b8b93] font-medium text-right">Comp. Index</th>
                      <th className="px-4 py-3 text-[#8b8b93] font-medium text-right">CPC</th>
                      <th className="px-4 py-3 text-[#8b8b93] font-medium">Trend (12mo)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#2a2a2a]">
                    {results.map((item, i) => (
                      <tr key={i} className="hover:bg-[rgba(255,255,255,0.05)] transition-colors">
                        ...
                        <td className="px-4 py-3">{renderTrend(item.monthly_searches)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
