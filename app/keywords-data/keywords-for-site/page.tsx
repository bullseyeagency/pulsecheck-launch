...
export default function KeywordsForSitePage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/keywords-data/keywords-for-site', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ target: target.trim(), location_code: locationCode, language_code: languageCode }),
      });
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `keywords-for-site-${target.trim().replace(/[^a-z0-9]/gi, '-')}.csv`;
    a.click();
      ...
      <div className="ml-64 min-h-screen bg-[#0a0a0a]">
        <div className="p-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-2xl font-bold text-white">Keywords for Site</h1>
          </div>
          <p className="text-[#a1a1aa] mb-8">
            Discover keywords that a website ranks for or is relevant to.
          </p>
          ...
          <form onSubmit={handleSearch} className="bg-[#141414] border border-[#2a2a2a] rounded p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm text-[#a1a1aa] mb-1.5">Target URL</label>
                <input
                  type="text"
                  value={target}
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              >
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Find Keywords'}
              </button>
              {results.length > 0 && (
                <button
                  type="button"
          ...
          {results.length > 0 && (
            <div className="bg-[#141414] border border-[#2a2a2a] rounded overflow-hidden">
              <div className="px-6 py-3 border-b border-[#2a2a2a]">
                <span className="text-sm text-[#a1a1aa]">{results.length} keywords found</span>
              </div>
          ...
          {/* Empty state */}
          {!loading && !error && results.length === 0 && target.trim() && (
            <div className="text-center py-12 text-[#8b8b93] text-sm">
              No keywords found for this site.
            </div>
          )}
        </div>
      </div>
