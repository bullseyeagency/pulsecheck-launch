...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      'DataForSEO SERP google-autocomplete response:',
    ...
    console.error('Error fetching google autocomplete SERP:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to fetch google autocomplete SERP' },
      { status: 500 }
    );
  }
}
