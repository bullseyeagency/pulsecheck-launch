...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('DataForSEO keyword-overview response:', data.tasks?.[0]?.status_code, data.tasks?.[0]?.status_message);
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to fetch keyword overview' },
      { status: 500 }
    );
  }
}
