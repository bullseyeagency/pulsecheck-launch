...
export async function POST(request: NextRequest) {
  try {
    ...
    const { product_id, location_code = 2840, language_code = 'en' } = body as {
      product_id: string;
    ...
    const taskRes = await fetch(
      'https://api.dataforseo.com/v3/merchant/google/reviews/task_post',
      { method: 'POST', headers, body: JSON.stringify(taskPayload) }
    );

    console.log('Merchant reviews task_post status:', taskRes.status);
    ...
    console.log('Merchant reviews task ID:', taskId);
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      const resultRes = await fetch(
        `https://api.dataforseo.com/v3/merchant/google/reviews/task_get/advanced/${taskId}`,
        { method: 'GET', headers }
      );

      console.log(`Merchant reviews poll attempt ${i + 1}:`, resultRes.status);
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to fetch merchant reviews' },
      { status: 500 }
    );
  }
}
