...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Amazon products task_post status:', taskRes.status);
    ...
    console.log('Amazon products task ID:', taskId);
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      console.log(`Amazon products poll attempt ${i + 1}:`, resultRes.status);
      ...
      if (statusCode && statusCode !== 40601) {
        console.log('Amazon products task status:', statusCode, resultData.tasks?.[0]?.status_message);
      }
    }
