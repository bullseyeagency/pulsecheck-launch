...
export default function BacklinkCompetitorsPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/backlinks/competitors', {
        method: 'POST',
  ...
  const exportCSV = () => {
    ...
    const headers = ['Domain', 'Rank', 'Backlinks', 'Referring Domains'];
    ...
    a.href = url;
    a.download = `backlink-competitors-${target.trim()}.csv`;
    a.click();
      ...
      <div className="ml-64 min-h-screen bg-[#0a0a0a]">
        <div className="p-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-2xl font-bold text-white">Backlink Competitors</h1>
          </div>
          <p className="text-[#a1a1aa] mb-8">
            Find domains that compete for the same backlinks as your target.
          </p>
          ...
          {results.length > 0 && (
            <div className="bg-[#141414] border border-[#2a2a2a] rounded overflow-hidden">
              <div className="px-6 py-3 border-b border-[#2a2a2a]">
                <span className="text-sm text-[#a1a1aa]">{results.length} competitors found</span>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-[#1a1a1a] text-left">
                      ...
                      <th className="px-4 py-3 text-[#8b8b93] font-medium">Referring Domains</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#2a2a2a]">
                    {results.map((item, i) => (
                      <tr key={i} className="hover:bg-[rgba(255,255,255,0.05)] transition-colors">
                        ...
                        <td className="px-4 py-3 text-white font-mono">{item.backlinks_num?.toLocaleString() ?? '-'}</td>
          ...
          {!loading && !error && results.length === 0 && target && (
            <div className="text-center py-12 text-[#8b8b93] text-sm">
              No competitors found. Try a different domain.
            </div>
          )}
        </div>
      </div>
