...
const STEP_DEFINITIONS: { name: string; icon: React.ComponentType<{ className?: string }>; field: string }[] = [
...
export default function AuditPage() {
  return (
    <Suspense fallback={
      <><Navbar /><main className="ml-64 min-h-screen bg-[#0a0a0a] p-8"><div className="max-w-3xl mx-auto"><Loader2 className="h-8 w-8 text-[#EF5744] animate-spin mx-auto mt-20" /></div></main></>
    }>
      <AuditPageInner />
    </Suspense>
  );
...
function AuditPageInner() {
  ...
  const pollAuditStatus = useCallback(
    (id: string) => {
      pollRef.current = setInterval(async () => {
        try {
          ...
          setSteps((prev) =>
            prev.map((step) => {
              const dbField = DB_FIELD_MAP[step.field];
              const value = audit[dbField];
              if (value === null || value === undefined) {
                // Show as running while audit is in progress
          ...
          if (audit.status === 'complete' || audit.status === 'failed') {
            ...
            if (audit.status === 'complete') {
              setState('complete');
              setTimeout(() => router.push(`/audit/${id}`), 1000);
            }
          }
        } catch {
  ...
  const statusIcon = (status: StepStatus['status']) => {
    switch (status) {
      case 'pending':
        return <span className="w-5 h-5 rounded-full border-2 border-[#2a2a2a] inline-flex" />;
      case 'running':
        return <Loader2 className="h-5 w-5 text-[#EF5744] animate-spin" />;
      case 'complete':
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-[#8b8b93] text-sm mt-1">
                Full-spectrum SEO analysis with actionable recommendations.
              </p>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
                    ...
                    <div className="flex gap-3">
                      <div className="flex-1 relative">
                        ...
                        <input
                          ...
                          required
                          className="w-full pl-10 pr-4 py-3 bg-[#0a0a0a] border-2 border-[#2a2a2a] text-white rounded focus:border-[#EF5744] outline-none placeholder:text-[#8b8b93] text-sm"
                        />
                      </div>
                      <button
                        type="submit"
                        className="px-8 py-3 bg-[#EF5744] hover:bg-[#c93a2a] text-white font-semibold rounded transition-colors text-sm flex items-center gap-2"
                      >
            ...
            {state === 'crawling' && (
              ...
              >
                ...
                <p className="text-[#8b8b93] text-sm mt-2">
                  Crawling {url} to detect industry and location.
                </p>
              </motion.div>
            )}
            ...
            {state === 'confirm' && (
              ...
              >
                <div className="bg-[#141414] border border-[#2a2a2a] rounded p-8">
                  <p className="text-[#8b8b93] text-xs uppercase tracking-wider mb-1">Analyzing</p>
                  <p className="text-white font-mono text-sm mb-6">{url}</p>

                  <div className="bg-[#0a0a0a] border border-[#2a2a2a] rounded p-4 mb-6">
                    <p className="text-[#a1a1aa] text-sm mb-4">
                      We detected the following. Edit if needed:
                    </p>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-xs font-medium text-[#8b8b93] uppercase tracking-wider mb-1.5">
                          Industry
                        </label>
                        <input
                          type="text"
                          ...
                          placeholder="e.g. Dentistry, Roofing, Legal"
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-[#EF5744] hover:bg-[#c93a2a] text-white font-semibold rounded transition-colors text-sm flex items-center justify-center gap-2"
                  >
                    Continue
                      ...
                      return (
                        <motion.div
                          ...
                          className={`flex items-center gap-3 px-4 py-3 rounded border transition-colors ${
                            step.status === 'running'
                              ? 'border-[#EF5744]/30 bg-[rgba(239,87,68,0.04)]'
                        ...
                        >
                          ...
                          <span
                            className={`flex-1 text-sm font-medium ${
                              step.status === 'pending' ? 'text-[#8b8b93]' : 'text-white'
                            }`}
                          >
                            {step.name}
                          </span>
            ...
            {state === 'complete' && (
              <motion.div
                ...
                className="bg-[#141414] border border-emerald-500/20 rounded p-12 text-center"
              >
                <CheckCircle className="h-12 w-12 text-emerald-500 mx-auto mb-4" />
                <p className="text-white text-lg font-semibold">Audit Complete</p>
                <p className="text-[#8b8b93] text-sm mt-2">Redirecting to report...</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
